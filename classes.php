...
if(!isset($_GET['category']) and !isset($_POST['category'])) {
	...
	if(has_capability(PERMISSION, $context)) {
		...
		if(is_array($classes)) {
			$c .= "<table>";
				/*<tr><td>".get_string('name', 'block_classmanager')."</td><td></td>
				<td>".get_string('numberofstudents', 'block_classmanager')."</td></tr>";*/
			foreach($classes as $class) {
				...
				$c .= "<tr><td>".$class->name."</td>
					<td><a href=\"".$CFG->wwwroot."/blocks/classmanager/editclass.php?category=".$categoryid."&classid=".$class->id."\">"
					.get_string('edit', 'block_classmanager')."</a></td>
