		...
		if(isset($_FILES['importfile'])) {
			...
			while(!feof($file)) {
				...
				if(count($param) == 5 || count($param)==4) {
					...
					/*print $newuser->mail;
					print "match mail:";
					print preg_match("/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $newuser->mail);
					print "<br>match class:". preg_match(	"/^([1-5])([A-Z]{1,4})/", $newuser->classe);*/
					//print $newuser->classe;
					if(
						//test if a regular class    ^([1-5])([A-Z]{1,4})
						/*($newuser->classe == 'TE' || $newuser->classe == 'PE' || 
								preg_match(	"/^([1-5])([A-Z]{1,4})/", $newuser->classe)) &&*/
						...
						$passcharsspecial=",.-;:_#+*!§$%&/()=?";
						$pass=substr(str_shuffle($passcharslower),0,2).substr(str_shuffle($passcharsupper),0,2).substr(str_shuffle($passcharsnumbers),0,2).substr(str_shuffle($passcharsspecial),0,2);
